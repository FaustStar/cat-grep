...
for flag in -b -e -n -s -t -v -T -E --number-nonblank --number --squeeze-blank -et -best -ent -TEsnv -bn; do
    echo "Test #$test_number"
    echo "Input check: s21_cat/cat $test_data/ascii.txt $flag $test_data/test3.txt"
    ./s21_cat $test_data/ascii.txt $flag $test_data/test3.txt > s21cat_result
    cat $test_data/ascii.txt $flag $test_data/test3.txt > cat_result
    if diff -s s21cat_result cat_result; then
        successed_number=$((successed_number + 1))
        echo "Result: SUCCESS"
    else
